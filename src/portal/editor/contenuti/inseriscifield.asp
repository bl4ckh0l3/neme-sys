...
function insertField(){
	if(document.form_inserisci.description.value == "") {
		alert("<%=langEditor.getTranslated("backend.contenuti.detail.js.alert.insert_description")%>");
		document.form_inserisci.description.focus();
		return false;		
	}else if(isSpecialChar(document.form_inserisci.description.value)) {
		alert("<%=langEditor.getTranslated("backend.contenuti.detail.js.alert.not_use_special_char")%>");
		document.form_inserisci.description.focus();
		return false;		
	}
	
	/*if(document.form_inserisci.id_group.value == "") {
		alert("<%=langEditor.getTranslated("backend.contenuti.detail.js.alert.insert_group")%>");
...
function deleteGroup(field){
	...
	if(confirm("<%=langEditor.getTranslated("backend.contenuti.lista.js.alert.delete_group")%>?")){
...
function insertGroup(){
	...
	if(group_order.value == "") {
		alert("<%=langEditor.getTranslated("backend.contenuti.detail.js.alert.insert_order")%>");
...
function addFieldValues(fieldValCounter){
	...
	$("#field_values_container"+counter).append($('<input type="text"/>').attr('name', "field_values"+counter).attr('class', "formFieldTXTMedium").attr('value', "").keypress(function(event) {return notSpecialCharAndSpace(event); }));
	var render='&nbsp;<a href="';
	render+="javascript:delFieldValues("+counter+",' ',' ','field_values_container"+counter+"',0);";
	render+='"><img src="<%=Application("baseroot")&"/editor/img/delete.png"%>" title="<%=langEditor.getTranslated("backend.contenuti.detail.table.alt.del_field_value")%>" alt="<%=langEditor.getTranslated("backend.contenuti.detail.table.alt.del_field_value")%>" hspace="5" vspace="0" border="0"></a><br/>';
...
function delFieldValues(counter,id_element, value, field, remove){
	...
	if(remove==1){
		...
		$.ajax({
			type: "POST",
			url: "<%=Application("baseroot") & "/editor/contenuti/deletefieldvalue.asp"%>",
			data: query_string,
			success: function() {
			$("#"+field).remove();
			}
...
<div id="backend-warp">
	...
	<div id="container">
		...
		<form action="<%=Application("baseroot") & "/editor/contenuti/ProcessField.asp"%>" method="post" name="form_inserisci" id="form_inserisci">
		...
		<tr>
		<td>	
		  <span class="labelForm"><%=langEditor.getTranslated("backend.contenuti.detail.table.label.description")%></span><br>
		  ...
		  <%=langEditor.getTranslated("backend.contenuti.detail.table.label.field_help_desc")%>
		  </div>
		  <br/><br/>	
		  
		  <div align="left" style="float:left;"><span class="labelForm"><%=langEditor.getTranslated("backend.contenuti.detail.table.label.group")%></span><br>
			...
			<option value="<%=x%>" <%if (idGroup = x) then response.Write("selected")%>><%if not(langEditor.getTranslated("backend.contenuti.detail.table.label.group."&objDispGroup(x).getDescription()) = "") then response.write(langEditor.getTranslated("backend.contenuti.detail.table.label.group."&objDispGroup(x).getDescription())) else response.write(objDispGroup(x).getDescription()) end if%></option>
			<%next
			end if%>
			</select>
		  </div>
		  <div align="left" style="float:left;text-align:left;padding-top:20px;padding-right:20px;">
		  <a href="javascript:deleteGroup(document.form_inserisci.id_group);"><img src="<%=Application("baseroot")&"/editor/img/cancel.png"%>" title="<%=langEditor.getTranslated("backend.contenuti.detail.table.alt.delete_group")%>" alt="<%=langEditor.getTranslated("backend.contenuti.detail.table.alt.delete_group")%>" hspace="5" vspace="0" border="0"></a>
		  </div>
		  <div align="left" style="text-align:left;display:block;">
			<span class="labelForm"><%=langEditor.getTranslated("backend.contenuti.detail.table.label.insert_group")%></span><br>
			<table>
			<tr><td>
			<%=langEditor.getTranslated("backend.contenuti.detail.table.label.group")%>&nbsp;<input type="text" name="group_value" id="group_value"  value="" class="formFieldTXTMedium" onkeypress="javascript:return notSpecialChar(event);">&nbsp;<a href="#" onMouseOver="javascript:showDiv('help_desc_group');" class="labelForm" onmouseout="javascript:hideDiv('help_desc_group');">?</a>
			  ...
			  <%=langEditor.getTranslated("backend.contenuti.detail.table.label.group_help_desc")%>
			  </div>
			</td></tr>
			<tr><td>
			<%=langEditor.getTranslated("backend.contenuti.detail.table.label.order")%>&nbsp;<input type="text" name="group_order" id="group_order" value="" class="formFieldTXTShort" maxlength="3" onkeypress="javascript:return isInteger(event);">
			&nbsp;&nbsp;<input type="button" class="buttonForm" hspace="2" vspace="4" border="0" align="absmiddle" value="<%=langEditor.getTranslated("backend.contenuti.detail.button.inserisci.label")%>" onclick="javascript:insertGroup();" />
		  </td></tr>
		  </table>
		  </div>
		  <br><br>
		  
		  <div align="left" style="float:left;padding-right:20px;"><span class="labelForm"><%=langEditor.getTranslated("backend.contenuti.detail.table.label.type")%></span><br>
			<select name="id_type" id="id_type" class="formFieldSelectSimple">
			<%for each x in typeList%>
			<option VALUE="<%=x%>" <%if not(typeField = "") AND (strComp(x, typeField, 1) = 0) then response.Write("selected")%>><%if not(langEditor.getTranslated("portal.commons.content_field.type.label."&typeList(x)) = "") then response.write(langEditor.getTranslated("portal.commons.content_field.type.label."&typeList(x))) else response.write typeList(x) end if%></option>
			<%next%>
			</select>
		  </div> 
 
		  <div align="left"><span class="labelForm"><%=langEditor.getTranslated("backend.contenuti.detail.table.label.type_content")%></span><br>
			<select name="id_type_content" id="id_type_content" class="formFieldSelectSimple">
			<%for each x in typeContentList%>
			<option VALUE="<%=x%>" <%if not(typeContent = "") AND (strComp(x, typeContent, 1) = 0) then response.Write("selected")%>><%if not(langEditor.getTranslated("portal.commons.content_field.type_content.label."&typeContentList(x)) = "") then response.write(langEditor.getTranslated("portal.commons.content_field.type_content.label."&typeContentList(x))) else response.write typeContentList(x) end if%></option>
			<%next%>
			</select>
		  </div>
			...
			$('#id_type').change(function() {
				...
				if(type_val_ch!=3){
					...
					$("select#id_type_content option[value=6]").remove();
				}else{
					...
					$("select#id_type_content").append($("<option></option>").attr("value",6).text("<%if not(langEditor.getTranslated("portal.commons.user_field.type_content.label.state_region") = "") then response.write(langEditor.getTranslated("portal.commons.user_field.type_content.label.state_region")) else response.write("state/region") end if%>"));
				}

				sortDropDownListByText("id_type_content");
			});
			
			$('#id_type_content').change(function() {
				var type_val_ch = $('#id_type').val();
				var typec_val_ch = $('#id_type_content').val();

				if((type_val_ch==3) && (typec_val_ch==5 || typec_val_ch==6)){
					$("#field_values_div").hide();
				}else if((type_val_ch== 3 || type_val_ch== 4) && (typec_val_ch!=5 && typec_val_ch!=6)){
					$("#field_values_div").show();
				}
			});
			</script>
		  ...
		  <div align="left" id="field_values_div">
			  <span class="labelForm"><%=langEditor.getTranslated("backend.contenuti.detail.table.label.values")%></span>&nbsp;<a href="#" onMouseOver="javascript:showDiv('help_desc_values');" class="labelForm" onmouseout="javascript:hideDiv('help_desc_values');">?</a>
			  ...
			  <%=langEditor.getTranslated("backend.contenuti.detail.table.label.field_help_desc_values")%>
			  </div>
			  &nbsp;<a href="javascript:addFieldValues(fieldValuesCounter);"><img src="<%=Application("baseroot")&"/editor/img/add.png"%>" title="<%=langEditor.getTranslated("backend.contenuti.detail.table.alt.add_field_value")%>" alt="<%=langEditor.getTranslated("backend.contenuti.detail.table.alt.add_field_value")%>" hspace="5" vspace="0" border="0"></a>
			  
			  <%On Error Resume Next
				Dim valuesCounter, totalCounter
				valuesCounter = 0
				totalCounter = 0
				Set objListValues = objField.getListContentFieldValues(id_field)
				...
				if(totalCounter > 0) then
					...
					for each j in arrKey%>
						...
						<br/><input type="text" name="field_values<%=valuesCounter%>" value="<%=j%>" class="formFieldTXT" onkeypress="javascript:return notSpecialCharAndSpace(event);">&nbsp;<a href="javascript:delFieldValues(<%=valuesCounter%>,'<%=id_field%>','<%=j%>','field_values_container<%=valuesCounter%>',1);"><img src="<%=Application("baseroot")&"/editor/img/delete.png"%>" title="<%=langEditor.getTranslated("backend.contenuti.detail.table.alt.del_field_value")%>" alt="<%=langEditor.getTranslated("backend.contenuti.detail.table.alt.del_field_value")%>" hspace="5" vspace="0" border="0"></a>
						<script>
						// aggiorno la mappa dei field
			  ...
			  <%if(valuesCounter = 0) then%>
				<span id="field_values_container<%=valuesCounter%>">
			  	<br/><input type="text" name="field_values<%=valuesCounter%>" value="" class="formFieldTXT" onkeypress="javascript:return notSpecialCharAndSpace(event);">&nbsp;<a href="javascript:delFieldValues(<%=valuesCounter%>,' ',' ','field_values_container<%=valuesCounter%>',0);"><img src="<%=Application("baseroot")&"/editor/img/delete.png"%>" title="<%=langEditor.getTranslated("backend.contenuti.detail.table.alt.del_field_value")%>" alt="<%=langEditor.getTranslated("backend.contenuti.detail.table.alt.del_field_value")%>" hspace="5" vspace="0" border="0"></a>
				<script>
				// aggiorno la mappa dei field
				fieldValuesMap.put(<%=valuesCounter%>,<%=valuesCounter%>);
				</script>
		  ...
		  <div align="left" id="max_lenght_div">
			  <span class="labelForm"><%=langEditor.getTranslated("backend.contenuti.detail.table.label.max_lenght")%></span><br>
			  <input type="text" name="max_lenght" value="<%=maxLenght%>" class="formFieldTXTShort" onkeypress="javascript:return isInteger(event);">
			  <br/><br/>
		  </div>			
		  <span class="labelForm"><%=langEditor.getTranslated("backend.contenuti.detail.table.label.order")%></span><br>
		  <input type="text" name="order" value="<%=order%>" class="formFieldTXTShort" maxlength="3" onkeypress="javascript:return isInteger(event);">
		  <br/><br/>	 	
		  <div align="left" style="float:left;padding-right:20px;"><span class="labelForm"><%=langEditor.getTranslated("backend.contenuti.detail.table.label.enabled")%></span><br>
		  ...
		  <div align="left" style="float:left;padding-right:20px;"><span class="labelForm"><%=langEditor.getTranslated("backend.contenuti.detail.table.label.required")%></span><br>
			...
			<option value="1"<%if ("1"=required) then response.Write(" selected")%>><%=langEditor.getTranslated("backend.commons.yes")%></option>	
			  ...
			  <%=langEditor.getTranslated("backend.contenuti.detail.table.label.field_help_required")%>
			  </div>
			<input type="hidden" value="" name="required_field_old" id="required_field_old">
		  </div>	 	
		  <div align="left"><span class="labelForm"><%=langEditor.getTranslated("backend.contenuti.detail.table.label.editable")%></span><br>
			<div id="editable">
		...
		var typec_val = $('#id_type_content').val();
		if((type_val== 3) && (typec_val==5 || typec_val==6)){
			$("#field_values_div").hide();
		}else if((type_val== 3 || type_val== 4) && (typec_val!=5 && typec_val!=6)){
			$("#field_values_div").show();
		}
		</script> 
		</td></tr>
		...
		<br/>
		<input type="button" class="buttonForm" hspace="2" vspace="4" border="0" align="absmiddle" value="<%=langEditor.getTranslated("backend.contenuti.detail.button.inserisci.label")%>" onclick="javascript:insertField();" />&nbsp;&nbsp;<input type="button" class="buttonForm" hspace="2" vspace="4" border="0" align="absmiddle" value="<%=langEditor.getTranslated("backend.commons.back")%>" onclick="javascript:location.href='<%=Application("baseroot")&"/editor/contenuti/Listanews.asp?cssClass=LN&showtab=contenutifield"%>';" />
		<br/><br/>
		<form action="<%=Application("baseroot") & "/editor/contenuti/ProcessField.asp"%>" method="post" name="form_inserisci_group">
		...
		<form action="<%=Application("baseroot") & "/editor/contenuti/ProcessField.asp"%>" method="post" name="form_delete_group">
