...
<div id="backend-warp">
	<div id="backend-content">
		...
		%>
		<br>
		<span class="labelMailSend"><%=langEditor.getTranslated("backend.utenti.mail.label.role")%>:</span>&nbsp;
		<%if(strRuolo = Application("admin_role")) then
			response.Write(Application("admin_const"))
		elseif (strRuolo = Application("editor_role")) then
			response.Write(Application("editor_const"))
		else
			response.Write(Application("guest_const"))
		end if%>
		<br><br>
	</div>
</div>
