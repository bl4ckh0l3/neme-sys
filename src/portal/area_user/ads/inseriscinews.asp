...
<!-- #include file="include/init3.inc" -->
...
function sendForm(saveEsc){
	if(controllaCampiInput()){
		...
		<%if(Application("use_aspupload_lib") = 1) then%>
			document.form_inserisci.action="<%=Application("baseroot") & "/area_user/ads/ProcessNews2.asp"%>";
		<%end if%>
...
function verificaData(yy, mm, dd, hh, minut, typeData){	
	if(yy.length < 4){
	  alert("<%=lang.getTranslated("backend.contenuti.detail.js.alert.insert_year_four")%>");
	  return false;		
	}else if (isNaN(yy)) {
	  alert("<%=lang.getTranslated("backend.contenuti.detail.js.alert.insert_year_number")%>");
	  return false;
	}else if (parseInt(yy) < 1900) {
	   alert("<%=lang.getTranslated("backend.contenuti.detail.js.alert.year_before_1900")%>");
	   return false;
	}else if(!checkDate(yy, mm, dd)){
	   alert("<%=lang.getTranslated("backend.contenuti.detail.js.alert.wrong_date")%>");
	   return false;		
	}else{
		if(typeData == "pub"){
...
function confrontaDate(dataInserimento, dataPubblicazione){
	if(dataInserimento.getTime() <= dataPubblicazione.getTime()){
		...
		return confirm('<%=lang.getTranslated("backend.contenuti.detail.js.alert.pub_date_less_curr_date")%>');
	}
}
...
<!-- #include virtual="/public/layout/area_user/grid_top.asp" -->
			...
			<tr> 		  		  
				<td>
					<form action="<%=Application("baseroot") & "/area_user/ads/ProcessNews.asp"%>" method="post" name="form_inserisci" enctype="multipart/form-data" accept-charset="UTF-8">
					...
					%>
					 ...
					 <input type="hidden" value="" name="abstract2">
					 <input type="hidden" value="" name="abstract3">
					...
					<%end if%>
					<!-- #include virtual="/area_user/ads/localization_widget.asp" -->
					...
					response.write(objT.renderTargetBox("listTargetCat", "targetcatbox_sx","targetcatbox_dx",lang.getTranslated("backend.contenuti.detail.table.label.target_x_contenuti_cat"), lang.getTranslated("backend.contenuti.detail.table.label.target_disp_cat"), "1", objTarget, objListaTargetPerUser, false, false, lang))
					Set objT = Nothing
					%>
					<br/><br/>						
					<%
					Set objT = New TargetClass
					response.write(objT.renderTargetBox("listTargetLang", "targetlangbox_sx","targetlangbox_dx",lang.getTranslated("backend.contenuti.detail.table.label.target_x_contenuti_lang"), lang.getTranslated("backend.contenuti.detail.table.label.target_disp_lang"), "3", objTarget, objListaTargetPerUser, false, false, lang))
					Set objT = Nothing
					%>						

					<br>
					<div class="divDetailHeader" align="left" onClick="javascript:showHideDiv('divAttachments');showHideDivArrow('divAttachments','arrowAttach');"><img src="<%if (Instr(1, typename(objFiles), "Dictionary", 1) > 0)then response.Write(Application("baseroot")&"/editor/img/div_freccia.gif") else response.Write(Application("baseroot")&"/editor/img/div_freccia2.gif") end if%>" vspace="0" hspace="0" border="0" align="right" id="arrowAttach"><%=lang.getTranslated("backend.news.view.table.label.attached_files")%></div>
					...
					<table border="0" cellspacing="0" cellpadding="0" class="principal" id="add_attach_table">
					  <tr>
						<td><span class="labelForm"><%=lang.getTranslated("backend.contenuti.detail.table.label.attachment")%></span></td>
						<td><span class="labelForm"><%=lang.getTranslated("backend.contenuti.detail.table.label.file_type_label")%></span></td>
					...
					if not(isNull(objFiles)) then%>		
						<table border="0" cellspacing="0" cellpadding="0" class="principal" id="modify_attach_table">
						  <tr>
							<td><span class="labelForm"><%=lang.getTranslated("backend.contenuti.detail.table.label.file_to_del")%></span></td>
							<td><span class="labelForm"><%=lang.getTranslated("backend.contenuti.detail.table.label.file_type_label")%></span></td>
							<td><span class="labelForm"><%=lang.getTranslated("backend.contenuti.detail.table.label.file_dida")%></span></td>
						  </tr>
						<%Dim objFilesInNews
					...
					<br><br>
					<div style="float:left;"><span class="labelForm"><%=lang.getTranslated("backend.contenuti.detail.table.label.stato_contenuto")%></span><br>
					<select name="stato_news" class="formFieldTXT">
					<option value="0" <%if (stato_news = 0) then response.Write("selected")%>><%=lang.getTranslated("backend.contenuti.lista.table.select.option.edit")%></option>
					<option value="1" <%if (stato_news = 1) then response.Write("selected")%>><%=lang.getTranslated("backend.contenuti.lista.table.select.option.public")%></option>
					</select>&nbsp;&nbsp;</div>
					<div>
					...
					<%if (Cint(id_news) <> -1) then%>
						...
						<a href="javascript:previewContent('<%=id_news%>')"><%=lang.getTranslated("backend.contenuti.detail.button.label.preview_contenuti")%></a>
						<br>
					<%end if%>
					</div><br><br>
					...
					<%if (Cint(id_news) <> -1) then%>
						...
						<a href="javascript:openWin('<%=Application("baseroot")&"/public/layout/include/popupInsertComments.asp?id_element="&id_news&"&element_type=1"%>','popupallegati',400,400,100,100);" title="<%=lang.getTranslated("backend.news.view.table.label.comments")%>"><img src="<%=Application("baseroot")&"/common/img/comment_add.png"%>" hspace="0" vspace="0" border="0"></a>
					<%else
			...
			<%if (Cint(id_news) <> -1) then%>		
				<form action="<%=Application("baseroot") & "/area_user/ads/DeleteNews.asp"%>" method="post" name="form_cancella_news">
			...
			%>	
		      ...
		      <form action="<%=Application("baseroot") & "/area_user/ads/InserisciNews.asp"%>" method="get" name="form_reload_page">
