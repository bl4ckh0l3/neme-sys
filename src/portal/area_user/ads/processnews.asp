...
if not(isEmpty(Session("objUtenteLogged"))) then
	...
	if (Cint(id_news) <> -1) then
			...
			for f = 1 to numMaxImgs			
				if not(isEmpty(Upload.UploadedFiles("fileupload"&f)))then
					...
					if (Instr(1, typename(fileXnews.getFileByFileNameAndIdNews(id_news,tmpFileName)), "File4NewsClass", 1) > 0) then
						call fileXnews.modifyFile(fileXnews.getFileByFileNameAndIdNews(id_news,tmpFileName).getFileID(),tmpFileName, Upload.UploadedFiles("fileupload"&f).Content_Type(), tmpFilePath, Upload.Form("fileupload"&f & "_dida"), Upload.Form("fileupload"&f & "_label"), objConn)
					else
						new_id_file = fileXnews.insertFile(tmpFileName, Upload.UploadedFiles("fileupload"&f).Content_Type(), tmpFilePath, Upload.Form("fileupload"&f & "_dida"), Upload.Form("fileupload"&f & "_label"), objConn)	
		...
		if(save_esc = 0) then
			redirectPage = Application("baseroot")&"/area_user/ads/inseriscinews.asp?id_news="&id_news
		end if
		...
		if (UBound(ks) <> -1) then
			...
			for q = 1 to numMaxImgs			
				if not(isEmpty(Upload.UploadedFiles("fileupload"&q)))then
					...
					new_id_file = fileXnews.insertFile(tmpFileName, Upload.UploadedFiles("fileupload"&q).Content_Type(), tmpFilePath, Upload.Form("fileupload"&q & "_dida"), Upload.Form("fileupload"&q & "_label"), objConn)
		...
		call UtenteXnews.insertUserXNews(Session("objUtenteLogged"), newMaxID, objConn)
		...
		if(save_esc = 0) then
			redirectPage = Application("baseroot")&"/area_user/ads/inseriscinews.asp?id_news="&newMaxID
		end if
		response.Redirect(redirectPage)				
	end if
