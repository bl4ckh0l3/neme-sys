...
if not(isEmpty(Session("objUtenteLogged"))) then
	...
	if (Cint(id_news) <> -1) then
		...
		if(save_esc = 0) then
			redirectPage = Application("baseroot")&"/area_user/ads/inseriscinews.asp?cssClass=LN&id_news="&id_news
		end if
		...
		if(save_esc = 0) then
			redirectPage = Application("baseroot")&"/area_user/ads/inseriscinews.asp?cssClass=LN&id_news="&newMaxID
		end if
		response.Redirect(redirectPage)				
	end if
