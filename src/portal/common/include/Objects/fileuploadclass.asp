...
const adModeReadWrite2 = 3
const adTypeBinary2 = 1
const adTypeText2 = 2
const adSaveCreateOverWrite2 = 2
...
Class FileUploadClass
	...
	Private Sub Class_Initialize()
		...
		StreamRequest.Type = adTypeText2
		StreamRequest.Open
		uploadedYet = false
	...
	public Function getVarArrayBinRequest()
		getVarArrayBinRequest = VarArrayBinRequest
	End Function
	...
	'Calls Upload to extract the data from the binary request
	...
	Public Sub Save(path)
		...
		For Each fileItem In UploadedFiles.Items
			...
			streamFile.Type = adTypeBinary2
			streamFile.Open
			StreamRequest.Position=fileItem.Start
			StreamRequest.CopyTo streamFile, fileItem.Length
			streamFile.SaveToFile filePath, adSaveCreateOverWrite2
			streamFile.close
	...
	public sub SaveOne(path, num, byref outFileName, byref outLocalFileName)
		...
		if UploadedFiles.Count > 0 then
			...
			StreamRequest.Position = fileItem.Start
			StreamRequest.CopyTo streamFile, fileItem.Length
			streamFile.SaveToFile path & outLocalFileName, adSaveCreateOverWrite2
			streamFile.close
			Set streamFile = Nothing
	...
	Public Sub SaveAttach(path, fieldNamePrefix, maxItem)
		Dim streamFile, fileItem, key
		...
		for j = 1 to maxItem
			...
			Set fileItem = UploadedFiles(CStr(fieldNamePrefix&j))
			Set streamFile = Server.CreateObject("ADODB.Stream")
			streamFile.Type = 1
			streamFile.Open
			StreamRequest.Position=fileItem.Start
	...
	'Calls Upload to extract the data from the binary request and then saves the uploaded files
	Public Sub SaveTemplatesFiles(objLangList, path_asp, path_css, path_img, path_include, path_js)
		Dim streamFile, fileItem, path_asp_lang

		if Right(path_asp, 1) <> "\" then path_asp = path_asp & "\"
		if Right(path_css, 1) <> "\" then path_css = path_css & "\"
		if Right(path_img, 1) <> "\" then path_img = path_img & "\"
		if Right(path_include, 1) <> "\" then path_include = path_include & "\"
		if Right(path_js, 1) <> "\" then path_js = path_js & "\"

		if not uploadedYet then Upload

		For Each fileItem In UploadedFiles.Items
			
			if(Instr(1, fileItem.FileName, ".inc", 1) > 0) then
				Set streamFile = Server.CreateObject("ADODB.Stream")
				streamFile.Type = 1
				streamFile.Open
				StreamRequest.Position=fileItem.Start
				StreamRequest.CopyTo streamFile, fileItem.Length
				streamFile.SaveToFile path_include & fileItem.FileName, 2			
				streamFile.close
				Set streamFile = Nothing	
				...
				For Each lang In objLangList
					...
					StreamRequest.Position=fileItem.Start
					StreamRequest.CopyTo streamFile, fileItem.Length
					path_include_lang = path_asp & Ucase(objLangList(lang).getLanguageDescrizione()) & "\" &"include\"
					streamFile.SaveToFile path_include_lang & fileItem.FileName, 2			
					streamFile.close
					Set streamFile = Nothing
				Next
			elseif(Instr(1, fileItem.FileName, ".js", 1) > 0) then
				Set streamFile = Server.CreateObject("ADODB.Stream")
				streamFile.Type = 1
				streamFile.Open
				StreamRequest.Position=fileItem.Start
				StreamRequest.CopyTo streamFile, fileItem.Length
				streamFile.SaveToFile path_js & fileItem.FileName, 2			
				streamFile.close
				Set streamFile = Nothing	
			else
				Select Case fileItem.ContentType
				Case "image/gif"
					Set streamFile = Server.CreateObject("ADODB.Stream")
					streamFile.Type = 1
					streamFile.Open
					StreamRequest.Position=fileItem.Start
					StreamRequest.CopyTo streamFile, fileItem.Length
					streamFile.SaveToFile path_img & fileItem.FileName, 2			
					streamFile.close
					Set streamFile = Nothing	
				Case "image/pjpeg"
					Set streamFile = Server.CreateObject("ADODB.Stream")
				...
				Case "text/css"
					...
					StreamRequest.Position=fileItem.Start
					StreamRequest.CopyTo streamFile, fileItem.Length
					streamFile.SaveToFile path_css & fileItem.FileName, 2			
					streamFile.close
					Set streamFile = Nothing	
				Case Else			
					Set streamFile = Server.CreateObject("ADODB.Stream")
					...
					StreamRequest.Position=fileItem.Start
					StreamRequest.CopyTo streamFile, fileItem.Length
					streamFile.SaveToFile path_asp & fileItem.FileName, 2			
					streamFile.close
					Set streamFile = Nothing
					...
					For Each lang In objLangList
						...
						StreamRequest.Position=fileItem.Start
						StreamRequest.CopyTo streamFile, fileItem.Length
						path_asp_lang = path_asp & Ucase(objLangList(lang).getLanguageDescrizione()) & "\"
						streamFile.SaveToFile path_asp_lang & fileItem.FileName, 2			
						streamFile.close
						Set streamFile = Nothing
					Next
				End Select					
	...
	Private Function ConvertUtf8BytesToString(start, length)	
	    ...
	    objStream.Mode = adModeReadWrite2
	    objStream.Type = adTypeBinary2
	    objStream.Open
...
Class UploadedFile
    ...
    Public Property Get Content_Type()
    ...
    Public Property Get FileLength()
        FileLength = Length
    End Property
