...
<%
   ...
   response.write "<html><body><h3>RC4 Test Harness</h3><b>Plaintext was:</b> " & txt & "<br><br>"
   ...
   cryptedHex = ""
   for x = 1 to len(strTemp)
     cryptedHex = cryptedHex & hex(asc(mid(strTemp, x, 1)))
   next
   response.write cryptedHex
   response.write "<br>"
   cryptedDec = ""
   for x = 1 to len(strTemp)
      cryptedDec = cryptedDec & chr("&H" & mid(cryptedHex,x,2))
   next
   response.write server.urlencode(cryptedDec)
   response.write "<br>"
   
   'txt = EnDeCrypt(strTemp, psw)
   txt = EnDeCrypt(cryptedDec, psw)
