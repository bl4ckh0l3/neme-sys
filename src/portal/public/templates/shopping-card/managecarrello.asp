...
gerarchia= Upload.Form("gerarchia")
resetQta = false
...
if(not(isNull(objCarrelloUser))) then
		...
		response.Redirect(Application("baseroot")&Application("dir_upload_templ")&"shopping-card/card.asp?id_carrello=" & objCarrelloUser.getIDCarrello()&"&id_ads="&id_ads&"&gerarchia="&gerarchia&Request.ServerVariables("QUERY_STRING"))
else
