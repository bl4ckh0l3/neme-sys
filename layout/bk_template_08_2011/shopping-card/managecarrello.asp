...
Dim id_prodotto, operation, qta_prod, prezzo, totale_prod, id_carrello, counter_prod, form_counter
order_carrello_by = 3
id_carrello = -1
...
if(not(isNull(objCarrelloUser))) then
		...
		Select Case operation
		   Case "add"
			  ...
			  call objProdPerCarr.addItem(objCarrelloUser.getIDCarrello(), id_prodotto, numCounterProd, qta_prod, objConn)
		   Case "del"
		...
		response.Redirect(Application("baseroot")&Application("dir_upload_templ")&"shopping-card/carrello2.asp?id_carrello=" & objCarrelloUser.getIDCarrello())
else
