			...
			<table class="tableContentLista" border="0" cellspacing="0" cellpadding="0">
			  <tr>
				...
				'for newsCounter = 0 to objListaNews.Count -1
					Dim objListaFilePerNews, objFileXNews, iTypeFile, bolHasSmallImg
					bolHasSmallImg = false
						...
						if not(isEmpty(objListaFilePerNews)) then						
							for each xObjFile in objListaFilePerNews
								...
								if(Cint(iTypeFile) = 1) then
									bolHasSmallImg = true%>	
									<a class="titoloNews" href="<%=Application("baseroot") & "/common/include/Controller.asp?gerarchia="&strGerarchia&"&id_news="&objSelNews.getNewsID()&"&page="&numPage%>"><img src="<%=Application("dir_upload_news")&objFileXNews.getFilePath()%>" width="100" height="100" hspace="3" vspace="3" border="0" alt="<%=objSelNews.getTitolo()%>" /></a>
									<%Exit for
								end if
								Set objFileXNews = nothing	
							next
							
							if not(bolHasSmallImg) then%>
								<a class="titoloNews" href="<%=Application("baseroot") & "/common/include/Controller.asp?gerarchia="&strGerarchia&"&id_news="&objSelNews.getNewsID()&"&page="&numPage%>"><img src="<%=Application("baseroot") & "/common/img/non_disponibile.gif"%>" width="100" height="100" hspace="3" vspace="3" border="0" alt="<%=objSelNews.getTitolo()%>" /></a>	
							<%end if
						else%>
							<a class="titoloNews" href="<%=Application("baseroot") & "/common/include/Controller.asp?gerarchia="&strGerarchia&"&id_news="&objSelNews.getNewsID()&"&page="&numPage%>"><img src="<%=Application("baseroot") & "/common/img/non_disponibile.gif"%>" width="100" height="100" hspace="3" vspace="3" border="0" alt="<%=objSelNews.getTitolo()%>" /></a>
						<%end if
