...
<%if not(isEmpty(Session("objUtenteLogged"))) then
	...
	Set objUsrLoggedEditorTmp = new UtenteCLass
	Set objUsrLoggedEditor = objUsrLoggedEditorTmp.findUserByID(Session("objUtenteLogged"))
...
<table class="tableContainer" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<table class="tableContent" border="0" align="left" cellpadding="0" cellspacing="0">
	  <tr>
		<td colspan="3" class="tdContentColspan">
		<img src="<%=Application("baseroot")&Application("dir_upload_templ")&"templateAreaRiservata/img/header.jpg"%>" vspace="0" hspace="0" border="0" align="top">		
		</td>
		</tr>
	  <tr>
	  ...
	  <tr>
		<td class="tdContentSx"><img src="<%=Application("baseroot")&Application("dir_upload_templ")&"templateAreaRiservata/img/pulsante_arancione.gif"%>" vspace="32" hspace="0" border="0" align="top"></td>
		<td class="tdContent">
		<%	
		Dim bolHasObj
		bolHasObj = false
		...
		if bolHasObj then
			...
			response.Write("<div align=""center""><b>"& lang.getTranslated("portal.commons.templates.label.page_in_progress")&"</b><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br></div>")
		end if%>
		</td>
		<td class="tdContentDx">
		<img src="<%=Application("baseroot")&Application("dir_upload_templ")&"templateAreaRiservata/img/logo_santenini_small.gif"%>" vspace="0" hspace="0" border="0"><br>
		<img src="<%=Application("baseroot")&"/common/img/spacer.gif"%>" height="20" vspace="0" hspace="0" border="0"><br>
		<%if(bolHasObj) then%>
			<!-- #include file="templateAreaRiservata/include/fileAllegati.inc" -->
			<%Set objCurrentNews = nothing
		end if%>
		</td>
	  </tr>
	  <tr>
		<td colspan="2" class="tdContentColspan"><!-- #include virtual="/common/include/MenuFruizioneBottom.inc" --></td>
...
if(Err.number <> 0) then
	...
	response.Redirect(Application("baseroot")&"/login.asp?from=reserved_area&gerarchia="&request("gerarchia"))
end if
%>