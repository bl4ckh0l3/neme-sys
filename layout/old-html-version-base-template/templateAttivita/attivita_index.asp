		...
		if(level > 1) then
			...
			if(iGerDiff <= 1) then
				if (InStr(1, Left(x, iLenGer-3), strSubTmpGerFiltered, 1) > 0) then
					...
					if not(isNull(lang.getTranslated(menuCompleteCatDescTrans))) AND not(lang.getTranslated(menuCompleteCatDescTrans) = "") then%><img align="center" src="<%=Application("baseroot")&"/common/img/"&lang.getTranslated(menuCompleteCatDescTrans)&".jpg"%>" border="0" name="<%=lang.getTranslated(menuCompleteCatDescTrans)&"_attivita"%>" id="<%=lang.getTranslated(menuCompleteCatDescTrans)&"_attivita"%>" hspace="0" vspace="0"/><%else response.Write(menuComplete(x)) end if%><%end if
			end if
		end if	
	next
		...
		if(level > 1) then
			...
			if(iGerDiff <= 1) then
				if (InStr(1, Left(x, iLenGer-3), strSubTmpGerFiltered, 1) > 0) then
					...
					Set objCategoriaCheck = nothing				
					%><a href="<%=strHref%>"><%if not(isNull(lang.getTranslated(menuCompleteCatDescTrans))) AND not(lang.getTranslated(menuCompleteCatDescTrans) = "") then%><img align="center" src="<%=Application("baseroot")&"/common/img/"&lang.getTranslated(menuCompleteCatDescTrans)%><%if(strComp(x, strSubTmpGer, 1) = 0) then response.Write("_on")%><%=".gif"%>" border="0" name="<%=lang.getTranslated(menuCompleteCatDescTrans)&"_attivita2"%>" id="<%=lang.getTranslated(menuCompleteCatDescTrans)&"_attivita2"%>" onMouseOver="MM_swapImage('<%=lang.getTranslated(menuCompleteCatDescTrans)&"_attivita2"%>','','<%=Application("baseroot")&"/common/img/"&lang.getTranslated(menuCompleteCatDescTrans)&"_on.gif"%>',1)" onMouseOut="MM_swapImgRestore()" hspace="0" vspace="0"/><%else response.Write(menuComplete(x)) end if%></a><%end if
			end if
		end if	
	next
