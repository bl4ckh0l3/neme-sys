...
Dim stato
stato = 1
if not(request("is_preview_content") = "") AND request("is_preview_content") = "1" then
	stato = null
end if
		...
		if bolHasObj then
			...
			<table class="tableContentLista" border="0" cellpadding="0" cellspacing="0">			 
			  <tr>
			    ...
			    <br />
			    <%if (Len(objCurrentNews.getAbstract1()) > 0) then%><span class="testoInfo"><%=objCurrentNews.getAbstract1()%></span><br /><br /><%end if%>
			    <%if (Len(objCurrentNews.getAbstract2()) > 0) then%><%=objCurrentNews.getAbstract2()%><%end if%>	
			     <span class="testoInfo"><%=objCurrentNews.getTesto() %></span>
			     </p></td>
