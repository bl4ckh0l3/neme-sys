	...
	<table class="tableContent" border="0" align="left" cellpadding="0" cellspacing="0">
	  <tr>
		...
		if bolHasObj then
			...
			<table class="tableContentLista" border="0" cellpadding="0" cellspacing="0">			 
			  <tr>
			    <td valign="top"><br/><p>
				<span class="titoloPartner"><%=objCurrentNews.getTitolo()%></span><br />
				<br />
			    
				<%					
				if (Len(objCurrentNews.getAbstract1()) > 0) then %>
					<span class="testoInfo"><%=objCurrentNews.getAbstract1()%></span><br /><br />
				<% end if
				if (Len(objCurrentNews.getAbstract2()) > 0) then %>
					<span class="titoloNews"><%=objCurrentNews.getAbstract2()%></span><br/><br/>
				<%end if
				if (Len(objCurrentNews.getAbstract3()) > 0) then%> 
					<%=objCurrentNews.getAbstract3()%>
				<%end if%>					
				</p>				
			
				<a href="javascript:openWin('<%=Application("baseroot")&"/common/include/popupInsertNewsComments.asp?id_news="&objCurrentNews.getNewsID()%>','popupallegati',400,400,100,100);" title="<%=lang.getTranslated("portal.templates.commons.label.see_comments_news")%>"><img src="<%=Application("baseroot")&"/common/img/bot_commenta.jpg"%>" hspace="0" vspace="0" border="0"></a><!--<a href="<%'=Application("baseroot") & "/common/include/Controller.asp?gerarchia=-5&destination=" & Application("dest_speciali")%>"><img src="<%'=Application("baseroot")&"/common/img/bot_mail.jpg"%>" alt="<%'=lang.getTranslated("cat_contact_special")%>" hspace="0" vspace="0" border="0" /></a>-->
			    </td>
			  ...
			  <tr>
			    <td colspan="2"><p class="testoInfo"><%=objCurrentNews.getTesto()%></p><br/><br/><br/><br/><br/><br/></td>
			    </tr>
			</table>			
