...
if(sendmail = 1) then
	...
	call objMail.sendMailContact(userMail, mailObject, mailText, destination, mailTo)
	Set objMail = Nothing	
	boolMailSent = true
...
function controllaCampiMail(){
	...
	if(strMail != ""){
		...
		alert("<%=lang.getTranslated("frontend.template_form.js.insert_mail")%>");
		document.form_send_mail.email.focus();
		return false;
	}	
	
	if(document.form_send_mail.oggetto.value == ""){
		alert("<%=lang.getTranslated("frontend.template_form.js.insert_oggetto")%>");
		document.form_send_mail.oggetto.focus();
		return false;
	}	
	
	if(document.form_send_mail.testo.value == ""){
		alert("<%=lang.getTranslated("frontend.template_form.js.insert_testo")%>");
  ...
  <tr>
	...
	<table class="tableContent" border="0" align="left" cellpadding="0" cellspacing="0">
	  <tr>
		...
		<table class="tableFormDettaglio" width="0" border="0" cellpadding="0" cellspacing="0" align="center">
		<tr>
		<td align="center" class="tableFormDettaglioCnt" valign="middle">
		<%	
		Dim bolHasObj
		bolHasObj = false
		...
		<%if (boolMailSent) then
			...
			response.write(lang.getTranslated("frontend.menu.label."&destination) & "<br><br>")%>	
						
			<form action="<%=Application("baseroot") &Application("dir_upload_templ")&"templateFormMail/Form_dettaglio.asp"%>" method="post" name="form_send_mail">
			  <input type="hidden" name="gerarchia" value="<%=strGerarchia%>">
			  <input type="hidden" name="destination" value="<%=lang.getTranslated("frontend.menu.label."&destination)%>">	
			  <input type="hidden" name="mailTo" value="<%=lang.getTranslated("frontend.mail.receiver."&destination)%>">			  
			  ...
			  <textarea name="testo" rows="10" class="formFieldTXTAREA"></textarea><br>
			  <div align="center">
			  <a href="javascript:sendMail();"><img src=<%=Application("baseroot")&"/common/img/formDettaglio_bottone.png"%> vspace="2" hspace="2" border="0" align="middle"></a>	
			  </div>
			</form>				
		<%end if%>
		</td></tr>
		<tr><td height="49" class="tableFormDettaglioDown" valign="bottom"></td></tr>
		</table>
		<br>
		</td>
