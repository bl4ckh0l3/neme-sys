...
thisPageName = "produzione_carrello.asp"
...
<table class="tableContainer" border="0" cellpadding="0" cellspacing="0" align="left">
  <tr>
	...
	<img src="<%=Application("baseroot")&Application("dir_upload_templ")&"templateProduzione/img/"&lang.getTranslated("frontend.template_pezziunici.label.txt_intro")&".gif"%>" align="left" vspace="0" hspace="0" border="0">
	</td>
	<td>	
    ...
    <td class="tdContainerContentCard">	
		<table class="tableContainerInner" border="0" cellpadding="0" cellspacing="0" align="left">
		  <tr>
			...
			<a href="javascript:window.close();"><img src="<%=Application("baseroot")&Application("dir_upload_templ")&"templateProduzione/img/"&lang.getTranslated("frontend.template_pezziunici.label.img_add_to_card")&".gif"%>" align="left" vspace="0" hspace="0" border="0"></a>
			</td>
			...
			<%if not(isNull(Session("pezziuniciCarrello"))) AND not(Session("pezziuniciCarrello") = "") then%>
			  ...
			  <tr><td>			
				<form action="<%=Application("baseroot") &Application("dir_upload_templ")&"templateProduzione/produzione_confirm.asp"%>" method="post" name="form_send_mail">
							...
							if (cbool(hasCardImg)) then%>
								<%for each xObjFile in objListaFilePerNews
									...
									if(objFileXNews.getFileTypeLabel() = 7) then%>	
										...
										&nbsp;<a class="txtRemoveFromList" href="<%=Application("baseroot") &Application("dir_upload_templ")&"templateProduzione/produzione_carrello.asp?gerarchia="&strGerarchia&"&id_news="&objSelNews.getNewsID()&"&remove=1"%>"><%=lang.getTranslated("frontend.template_pezziunici.label.remove_item")%></a></div>
										</div>
										<%Exit for
									end if
									Set objFileXNews = nothing	
				...
				next%>
				  ...
				  <input type="image" name="reset" onclick="return resetForm();" src="<%=Application("baseroot")&Application("dir_upload_templ")&"templateProduzione/img/"&lang.getTranslated("frontend.template_pezziunici.button.cancel.label")&".gif"%>" vspace="0" align="absmiddle">&nbsp;<input type="image" name="submit" onclick="return sendMail();" src="<%=Application("baseroot")&Application("dir_upload_templ")&"templateProduzione/img/"&lang.getTranslated("frontend.template_pezziunici.button.send.label")&".gif"%>" vspace="0" align="absmiddle">
				  </td></tr>
				  </table>
				</form>					
			  </td></tr>
