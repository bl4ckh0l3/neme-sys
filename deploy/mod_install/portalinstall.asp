...
installFormPageVar = publicInstallDirVar & "portalinstall.asp"
installResultPageVar = "/installed.asp"
globalQueryFile = publicInstallDirVar&"global_install_query.sql"
...
if(request("apply") = "true") then	
	...
	Application("srt_dbconn") = strDbConnVar
	Application("srt_default_server_name") = Request.ServerVariables("SERVER_NAME")
	...
	for each x in allAppVar
		...
		'configFile.writeLine("<"&objConf.getKey()&" attr_"&objConf.getKey()&"="""&objConf.getValue()&"""><![CDATA["&langEditor.getTranslated(objConf.getDescrizione())&"]]></"&objConf.getKey()&">")
	...
	if (objFSO.FolderExists(installDirVar)) then
		...
		call objFSO.DeleteFile(installDirVar&"\portalinstall.asp",true)
		call objFSO.DeleteFile(installDirVar&"\global_install_query.sql",true)
	end if	
	
	Set objFSO = nothing
	
	'************* IMPOSTO L'OGGETTO Application("objConn") CON L'OGGETTO objConn
