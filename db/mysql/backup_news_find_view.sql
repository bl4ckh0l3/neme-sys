...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `news_find` AS select `news`.*,`news_x_utente`.`id_utente` AS `id_utente` from (`news_x_utente` join `news` on((`news_x_utente`.`id_news` = `news`.`id`)));